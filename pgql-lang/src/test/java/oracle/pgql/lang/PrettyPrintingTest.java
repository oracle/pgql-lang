...
public class PrettyPrintingTest extends AbstractPgqlTest {
	...
	private void checkRoundTrip(String query1) throws PgqlException {
		...
		/*
		 * First, assert that when parsing a query into a GraphQuery object and then
		 * pretty printing that GraphQuery object, we obtain a string that is a valid
		 * PGQL query.
		 */
		...
		/*
		 * Since pretty-printed queries are in normal form, we can now round trip
		 * endlessly. Here, we assert that when pretty-printing a GraphQuery object that
		 * was parsed from a pretty-printed query, we obtain another GraphQuery object
