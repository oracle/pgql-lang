...
PGQL furthermore deeply integrates graph pattern matching with subquery capabilities, allowing vertices and edges that are matched in one query to be passed into another query for continued joining or graph pattern matching.
Finally, since PGQL is built on top of SQL's foundation, it benefits from all existing SQL features and any new SQL features that will be added to the standard over time.
