...
WHERE (n:Person WITH name = 'John') -[e1:friendOf]->
    (m:Person) <-[e2:belongs_to]- (o:Car)
```
...
### Syntactic Sugar for Topology Constraints

For user's convenience, PGQL provides several syntactic sugar (short-cuts) for topology constraints.
...
WHERE
  (n1) -> (m1),
  (n2) -> (m2) // vertices {n2, m2} are not connected to vertices {n1, m1},
